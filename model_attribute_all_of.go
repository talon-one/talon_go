/*
 * Talon.One API
 *
 * Use the Talon.One API to integrate with your application and to manage applications and campaigns:  - Use the operations in the [Integration API section](#integration-api) are used to integrate with our platform - Use the operation in the [Management API section](#management-api) to manage applications and campaigns.  ## Determining the base URL of the endpoints  The API is available at the same hostname as your Campaign Manager deployment. For example, if you access the Campaign Manager at `https://yourbaseurl.talon.one/`, the URL for the [updateCustomerSessionV2](https://docs.talon.one/integration-api#operation/updateCustomerSessionV2) endpoint is `https://yourbaseurl.talon.one/v2/customer_sessions/{Id}` 
 *
 * API version: 1.0.0
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package talon

import (
	"bytes"
	"encoding/json"
)

// AttributeAllOf struct for AttributeAllOf
type AttributeAllOf struct {
	EventTypeId *int32 `json:"eventTypeId,omitempty"`
}

// GetEventTypeId returns the EventTypeId field value if set, zero value otherwise.
func (o *AttributeAllOf) GetEventTypeId() int32 {
	if o == nil || o.EventTypeId == nil {
		var ret int32
		return ret
	}
	return *o.EventTypeId
}

// GetEventTypeIdOk returns a tuple with the EventTypeId field value if set, zero value otherwise
// and a boolean to check if the value has been set.
func (o *AttributeAllOf) GetEventTypeIdOk() (int32, bool) {
	if o == nil || o.EventTypeId == nil {
		var ret int32
		return ret, false
	}
	return *o.EventTypeId, true
}

// HasEventTypeId returns a boolean if a field has been set.
func (o *AttributeAllOf) HasEventTypeId() bool {
	if o != nil && o.EventTypeId != nil {
		return true
	}

	return false
}

// SetEventTypeId gets a reference to the given int32 and assigns it to the EventTypeId field.
func (o *AttributeAllOf) SetEventTypeId(v int32) {
	o.EventTypeId = &v
}

type NullableAttributeAllOf struct {
	Value AttributeAllOf
	ExplicitNull bool
}

func (v NullableAttributeAllOf) MarshalJSON() ([]byte, error) {
    switch {
    case v.ExplicitNull:
        return []byte("null"), nil
    default:
		return json.Marshal(v.Value)
	}
}

func (v *NullableAttributeAllOf) UnmarshalJSON(src []byte) error {
	if bytes.Equal(src, []byte("null")) {
		v.ExplicitNull = true
		return nil
	}

	return json.Unmarshal(src, &v.Value)
}
